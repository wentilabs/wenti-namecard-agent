...
async function telegramHandler(event) {
    ...
    try {
      ...
      if (message.photo) {
        ...
        await sendTelegramMessage(chatId, 'Extracting information from the image... This may take a few seconds.');
        ...
        if (response) {
          ...
          await sendTelegramMessage(chatId, "Sorry, I couldn't process that image. Please try again with a clearer image of a business card.");
        }
      ...
      return {
        ...
        try {
          if (chatId) {
            await sendTelegramMessage(chatId, "Sorry, an error occurred while processing your request. Please try again later.");
          }
        } catch (sendError) {
